...
const RadialCircularCalendar: React.FC<CalendarProps> = ({ initialDate, onSelectDate }) => {
  ...
  const handleDateSelect = (date: Date) => {
    ...
    // Calculate the rotation angle based on the selected date
    const dayIndex = date.getDate() - 1;
    const totalDays = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).getDate();
    const angle = (360 / totalDays) * dayIndex;

    setRotationAngle(angle); // Rotate the calendar to the selected date
  };
    ...
    <div className="relative w-96 h-96 mx-auto rounded-full bg-gradient-to-tl from-blue-800 via-teal-600 to-indigo-500 flex items-center justify-center overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          animate="visible"
          exit="exit"
          transition={{ duration: 0.3 }} // Shorter duration for smoother rotation
          className="relative top-[-5%] left-[-6%] w-full h-full"
          style={{
            transform: `rotate(${rotationAngle}deg)`, // Rotate the calendar based on selected date
            transition: "transform 0.3s ease-in-out", // Shortened transition time
          }}
        >
              ...
              className={`w-12 h-12 flex items-center justify-center text-base font-medium rounded-full transition-transform duration-300
                ...
                ${
                  selectedDate?.toDateString() === date.toDateString()
                    ? "bg-gradient-to-br from-blue-400 to-purple-500 text-white ring-4 ring-gray-800"
                    : "hover:bg-gray-400"
                }`}
            >
              {date.getDate()}
