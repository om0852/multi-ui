...
const LineChart = ({ data, config, xKey, yKeys, className }) => {
  ...
  return (
    <svg className={`w-full ${className}`} viewBox={`0 0 ${width} ${height}`}>
      ...
      {yKeys.map((key) => (
        <g key={key} onMouseEnter={() => setHoveredLine(key)} onMouseLeave={() => setHoveredLine(null)}>
          <motion.path
          ...
          />
          {hoveredLine === key && (
            <text x={width - margin.right} y={margin.top + yKeys.indexOf(key) * 20} fontSize="12" fill={config[key]?.color || "#000"} textAnchor="end">
              {config[key]?.label}
            </text>
          )}
        </g>
