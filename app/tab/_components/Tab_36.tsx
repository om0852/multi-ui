...
const TabsList: React.FC<TabsListProps> = ({ children, activeTab, setActiveTab, className = "" }) => {
  return (
    <div className={`inline-flex p-4 bg-gradient-to-br from-[#80D8FF] via-[#B388FF] to-[#EA80FC] bg-opacity-10 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(128,216,255,0.3)] ${className}`}>
      <div className="flex w-full gap-3 p-2 bg-black/10 rounded-xl backdrop-blur-xl">
        {React.Children.map(children, (child) =>
          React.isValidElement(child)
            ? React.cloneElement(child as React.ReactElement<any>, {
                activeTab,
