...
const TabsList: React.FC<TabsListProps> = ({ children, activeTab, setActiveTab, className = "" }) => {
  return (
    <div className={`inline-flex p-4 bg-gradient-to-br from-gray-700 via-gray-800 to-gray-900 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.2)] border border-gray-700/50 ${className}`}>
      <div className="flex w-full gap-3 p-2 bg-black/20 rounded-xl backdrop-blur-sm">
        {React.Children.map(children, (child) =>
          React.isValidElement(child)
            ? React.cloneElement(child as React.ReactElement<any>, {
                activeTab,
