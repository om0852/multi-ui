...
const TabsList: React.FC<TabsListProps> = ({ children, activeTab, setActiveTab, className = "" }) => {
  return (
    <div className={`inline-flex p-4 bg-gradient-to-br from-orange-500/20 via-red-500/20 to-rose-500/20 rounded-2xl backdrop-blur-xl border border-orange-500/10 ${className}`}>
      <div className="flex w-full gap-3 p-2 bg-black/10 rounded-xl">
        {React.Children.map(children, (child) =>
          React.isValidElement(child)
            ? React.cloneElement(child as React.ReactElement<any>, {
                activeTab,
