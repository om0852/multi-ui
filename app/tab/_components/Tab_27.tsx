...
const TabsList: React.FC<TabsListProps> = ({ children, activeTab, setActiveTab, className = "" }) => {
  return (
    <div className={`inline-flex p-4 bg-gradient-to-br from-green-800/30 via-emerald-800/30 to-teal-800/30 rounded-3xl backdrop-blur-xl border border-green-800/20 ${className}`}>
      <div className="flex w-full gap-3 p-2 bg-green-950/20 rounded-2xl">
        {React.Children.map(children, (child) =>
          React.isValidElement(child)
            ? React.cloneElement(child as React.ReactElement<any>, {
                activeTab,
