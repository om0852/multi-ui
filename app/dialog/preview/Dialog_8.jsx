...
function StyledDialogTrigger({ children, onClick }) {
  return (
    <button
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-70 backdrop-blur-sm z-50">
      <motion.div
        ...
        className="bg-gradient-to-br from-gray-100 to-gray-300 text-gray-800 rounded-2xl shadow-2xl w-full max-w-[90%] sm:max-w-xl p-6 sm:p-8 relative overflow-y-auto max-h-[90vh] border border-gray-400 backdrop-blur-lg"
      >
        <button
          onClick={onClose}
...
function StyledDialogHeader({ children }) {
  return <div className="mb-4 text-gray-800 text-lg font-semibold">{children}</div>;
}

function StyledDialogTitle({ children }) {
  return <h2 className="text-2xl font-bold text-teal-600">{children}</h2>;
}

function StyledDialogDescription({ children }) {
  return <p className="text-gray-600 mt-2">{children}</p>;
}

function StyledDialogFooter({ children }) {
  return <div className="mt-6 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-4">{children}</div>;
}
