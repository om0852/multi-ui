const RadialProgressBar = ({
  ...
  const angle = (currentProgress / 100) * 2 * Math.PI - Math.PI / 2; // Start from top
  const dotX = size / 2 + radius * Math.cos(angle);
  const dotY = size / 2 + radius * Math.sin(angle);
  
  // Calculate gradient rotation based on progress
  const gradientRotation = (currentProgress / 100) * 360;
  const gradientId = `gradient-${progressColor.replace('#', '')}`;

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">Radial Progress</h1>
        <p className="text-gray-600 mb-8">With moving indicator dot</p>
...
const RadialProgressBarDemo = () => {
  ...
  const handleAnimationStart = () => {
    console.log("Radial animation started");
  };
  
  const handleAnimationComplete = () => {
    console.log("Radial animation completed");
  };
