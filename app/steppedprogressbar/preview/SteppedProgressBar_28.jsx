...
const StepsRoot = ({
      ...
      {steps.map((step, index) => (
        <div key={index} className="flex items-center">
          ...
          {index < steps.length - 1 && (
            <motion.div
              className={`w-16 h-1 ${currentStep >= index ? 'bg-green-500' : 'bg-gray-300'}`}
              initial={{ width: 0 }}
  ...
  const StepsItem = ({ isActive, isCompleted, children }) => (
    ...
    >
      <motion.div
        className={`flex items-center justify-center w-14 h-14 rounded-full text-white font-semibold shadow-lg ${
          isCompleted
            ? 'bg-green-500'
            : isActive
            ? 'bg-blue-500'
  ...
  return (
    <div className="flex flex-col items-center space-y-12 p-8 w-full">
      <StepsList />
