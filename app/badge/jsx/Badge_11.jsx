...
export function NeonBadge({ text, color = 'bg-fuchsia-500' }) {
  return (
    <div
...
export function OrbitingDotsBadge({ text, color = 'bg-purple-600', dotColor = 'bg-pink-400' }) {
  const dots = new Array(6).fill(0);

  return (
    <div className="relative flex items-center justify-center">
      <span className={`relative z-10 px-6 py-3 text-white font-semibold rounded-full ${color}`}>{text}</span>
      {dots.map((_, i) => (
        <div
          key={i}
